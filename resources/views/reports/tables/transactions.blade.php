<table class="min-w-full divide-y divide-gray-200">
    <thead class="bg-gray-50">
        <tr>
            ...
            <th class="px-3 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wide">Detail Produk
            </th>
            <th class="px-3 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wide">Qty</th>
            ...
            @foreach ($trx->details as $idx => $detail)
                <tr>
                    ...
                    <td class="whitespace-nowrap px-3 py-3 text-sm text-gray-500">
                        {{ $idx === 0 ? $trx->transaction_date->format('d/m/Y') : '' }}</td>
                    <td class="whitespace-nowrap px-3 py-3 text-sm text-gray-500">
                        {{ $idx === 0 ? $trx->user->name ?? '-' : '' }}</td>
                    <td class="px-3 py-3 text-sm text-gray-500">
                        <div class="text-xs">{{ $detail->product->name ?? '-' }}</div>
                    </td>
                    <td class="whitespace-nowrap px-3 py-3 text-sm text-gray-500">
                        <div class="text-xs">{{ $detail->quantity }} {{ $detail->product->satuan ?? '-' }}</div>
                    </td>
