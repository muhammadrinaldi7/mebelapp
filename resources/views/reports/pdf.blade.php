...
<body>
    <div class="header">
        ...
        <p>
            Periode: {{ $dateFrom ? \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') : 'Awal' }} -
            ...
            <tbody>
                ...
                @foreach ($transactions as $trx)
                    @foreach ($trx->details as $idx => $detail)
                        @php
                            $subtotal = $detail->quantity * $detail->price_at_transaction;
                            $grandTotal += $subtotal;
                        @endphp
                        <tr>
                            ...
                            <td>
                                @if ($idx === 0)
                                    ...
                                    @elseif($trx->type === 'out')
                                        <span class="badge bg-orange">Keluar</span>
                                    @else
                                        <span class="badge bg-blue">Penjualan</span>
                                    @endif
                                @endif
                            </td>
                            ...
                            <td class="text-right">Rp {{ number_format($detail->price_at_transaction, 0, ',', '.') }}
                            </td>
        ...
        <table>
            ...
            <tbody>
                ...
                @foreach ($mutations as $mutation)
                    @php
                        $subtotal = $mutation->quantity * $mutation->price_at_transaction;
                        $grandTotal += $subtotal;
                    @endphp
                    <tr>
                        ...
                        <td>
                            ...
                            <br><span style="font-size:8px;color:#999;">SKU:
                                {{ $mutation->product->sku ?? '-' }}</span>
                        </td>
                        <td class="text-center">
                            ...
                            @elseif($mutation->transaction->type === 'out')
                                <span class="badge bg-orange">KELUAR</span>
                            @else
                                <span class="badge bg-blue">TERJUAL</span>
                            @endif
                        </td>
                        <td class="text-right font-bold">{{ $mutation->quantity }}</td>
                        <td class="text-right">Rp {{ number_format($mutation->price_at_transaction, 0, ',', '.') }}
                        </td>
                        <td class="text-right font-bold">Rp {{ number_format($subtotal, 0, ',', '.') }}</td>
                    </tr>
                @endforeach
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right font-bold">Rp {{ number_format($totalProfit - $totalDiscount, 0, ',', '.') }}
                    </td>
                </tr>
            </tbody>
        </table>
            ...
            <tbody>
                @foreach ($priceHistory as $hist)
                    <tr>
                        ...
                        <td class="text-right font-bold" style="color: #ef4444;">Rp
                            {{ number_format($hist->price_at_transaction, 0, ',', '.') }}</td>
                        <td class="text-right font-bold">Rp
                            {{ number_format($hist->price_at_transaction * $hist->quantity, 0, ',', '.') }}</td>
                    </tr>
                @endforeach
                @if (count($priceHistory) === 0)
                    <tr>
                        <td colspan="5" class="text-center">Belum ada riwayat transaksi barang masuk untuk produk ini
                            di rentang waktu terpilih.</td>
                    </tr>
                @endif
            </tbody>
        </table>
    ...
    <div class="footer">
        {{ config('app.name') }} &copy; {{ date('Y') }} — Laporan ini di-generate secara otomatis oleh Sistem
        Manajemen Stok Mebel.
    </div>
</body>
