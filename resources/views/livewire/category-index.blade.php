<div>
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Kelola data kategori mebel.</p>
        </div>
    ...
    <div class="mt-4">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari kategori..." class="block w-full sm:w-64 rounded-md border border-gray-300 px-3 py-2 text-gray-900 shadow-sm placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-600 sm:text-sm">
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-300">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                ...
                <tr>
                    ...
                    <td class="px-3 py-4 text-sm text-gray-500">{{ Str::limit($category->description, 50) ?? '-' }}</td>
                    <td class="px-3 py-4 text-sm text-center text-gray-500">{{ $category->products_count }}</td>
                    <td class="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium space-x-2">
                        ...
                        <button wire:click="confirmDelete({{ $category->id }})" class="text-red-600 hover:text-red-900">Hapus</button>
                    </td>
                </tr>
                @empty
                <tr><td colspan="4" class="py-4 text-center text-sm text-gray-500">Belum ada data kategori.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl ring-1 ring-gray-900/5">
                <h3 class="text-lg font-medium text-gray-900">{{ $editMode ? 'Edit Kategori' : 'Tambah Kategori' }}</h3>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl ring-1 ring-gray-900/5">
                ...
                <p class="mt-2 text-sm text-gray-500">Apakah Anda yakin ingin menghapus kategori ini?</p>
